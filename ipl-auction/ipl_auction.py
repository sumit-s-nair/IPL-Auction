...
connection = sqlite3.connect('ipl_players.db')
...
def bid_start():
    ...
    try:
        new_image_path = results[y][3]  # Fetching image path
        image = get_image_from_path(new_image_path)  # Using the path to load the image
...
def bid():
    def re(r):
        ...
        while i < 2:
            ...
            cursor.execute(f'SELECT COUNT(0) FROM {d[r]}')
            if cursor.fetchall()[0][0] != 15:  # If the team has fewer than 15 players
    ...
    def no():
        t = team.get()  # Get the team name
        cb2.configure(text=t)
        
        try:
            # Insert the current player's data into the selected team's table
            cursor.execute(f'INSERT INTO {d[r]} (ID, Name, BidAmount, ImagePath) VALUES (?, ?, ?, ?)', 
                           (results[y][0], results[y][1], results[y][2], results[y][3]))
            connection.commit()
    ...
    if z == 1:
        ...
        if name in d:
            try:
                # Insert current player into the selected team's table using parameterized query
                cursor.execute(f'INSERT INTO {d[name]} (ID, player_name, base_price, image_path) VALUES (?, ?, ?, ?)', 
                               (results[y][0], results[y][1], results[y][2], results[y][3]))
                connection.commit()
            except sqlite3.Error as e:
                print(f"An error occurred during insert: {e}")
...
def pas():
    ...
    while i < 2:
        ...
        if cursor.fetchall()[0][0] != 15:
            try:
                cursor.execute('INSERT INTO %s (ID, player_name, base_price, image_path) VALUES (?, ?, ?, ?)' % d[x], 
                               (results[y][0], results[y][1], results[y][2], results[y][3]))
                connection.commit()
...
def show_page3(page1,page2,page3):
    ...
    for j in d:
        ...
        for i in range(len(data)):
            n = data[i][1]
            new_image_path = data[i][3]
            new_image = get_image_from_path(new_image_path)
...
bg = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/background.png"), size = (1920,1080))
...
image1 = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/teams/RCB.png"),size=(230, 230))
...
image2 = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/teams/CSK.png"),size=(230, 230))
...
image3 = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/teams/MI.png"),size=(230, 230))
...
image4 = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/teams/SRH.png"),size=(230, 230))
...
image5 = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/teams/RR.png"),size=(230, 230))
...
image6 = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/teams/KKR.png"),size=(230, 230))
...
image7 = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/teams/PK.png"),size=(230, 230))
...
image8 = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/teams/DC.png"),size=(230, 230))
...
backim = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/icons/back.png"),size=(33, 35))
...
af(Totalam1)
imageam = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/icons/rupee.png"),size=(50, 50))
...
imagets = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/icons/amount-spent.png"),size=(50, 50))
...
imagebid = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/icons/bid.png"),size=(50, 50))
...
image_player = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/icons/profile.png"),size=(230, 230))
...
af(cb2)
cbim = ctk.CTkImage(dark_image=Image.open(r"./ipl-auction/assets/icons/profile.png"),size=(50, 50))
